    ...
    var SVGPanZoom = function() {
            ...
            setOptions: {
                ...
                this.options = {
                    ...
                    set limits(limits) {
                        ...
                        horizontal: {
                            var multiplier = Number(directionalLimits[0].replace(/%/g, '')) / 100;
        ...
        _createClass(SVGPanZoom, [{
            ...
            value: function validateLimits(viewBox) {
                ...
                if (viewBox.x < limits.minX)
                    viewBox.x = limits.minX;
                if (viewBox.y < limits.minY)
                    viewBox.y = limits.minY;
                if (viewBox.x + viewBox.width > limits.maxX)
                    viewBox.x = limits.maxX - viewBox.width;
                if (viewBox.y + viewBox.height > limits.maxY)
                    viewBox.y = limits.maxY - viewBox.height;
            }
        }, {
            key: 'reset',
